...
def _load_local_hf_imports():
    hf_imports_path = Path(__file__).resolve().parents[2] / "utils" / "hf_imports.py"
    spec = importlib.util.spec_from_file_location("mdt_local_hf_imports", hf_imports_path)
...
try:
    _hf_modules = _load_local_hf_imports().load_hf_diffusers_submodules(
        "configuration_utils",
        "models.modeling_utils",
        "utils",
    )
    _config_utils = _hf_modules["configuration_utils"]
    ConfigMixin = _config_utils.ConfigMixin
    register_to_config = _config_utils.register_to_config
    ModelMixin = _hf_modules["models.modeling_utils"].ModelMixin
    BaseOutput = _hf_modules["utils"].BaseOutput
...
@dataclass
class MDTTransformer2DModelOutput(BaseOutput):
...
class MDTTransformer2DModel(ModelMixin, ConfigMixin):
    """
    Masked Diffusion Transformer v2 for latent diffusion.
    """
    ...
    def forward(
    ...
    ) -> Union[MDTTransformer2DModelOutput, torch.Tensor]:
        if y is not None and class_labels is None:
            class_labels = y
        x = hidden_states
        t = timestep
        y = class_labels
        ...
        if not return_dict:
            return x
        return MDTTransformer2DModelOutput(sample=x)
...
def mdt_v2_xl_2(**kwargs):
    return MDTTransformer2DModel(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)

def mdt_v2_l_2(**kwargs):
    return MDTTransformer2DModel(depth=24, hidden_size=1024, patch_size=2, num_heads=16, **kwargs)

def mdt_v2_b_2(**kwargs):
    return MDTTransformer2DModel(depth=12, hidden_size=768, patch_size=2, num_heads=12, **kwargs)

def mdt_v2_s_2(**kwargs):
    return MDTTransformer2DModel(depth=12, hidden_size=384, patch_size=2, num_heads=6, **kwargs)


# Backward-compatible aliases
MDTv2 = MDTTransformer2DModel
MDTv2_XL_2 = mdt_v2_xl_2
MDTv2_L_2 = mdt_v2_l_2
MDTv2_B_2 = mdt_v2_b_2
MDTv2_S_2 = mdt_v2_s_2
