...
@dataclass
class RAEDDTTransformer2DModelOutput(BaseOutput):
...
class RAEDDTFinalLayer(nn.Module):
    """
...
class RAEDDTTransformer2DModel(ModelMixin, ConfigMixin):
    config_name = "config.json"
    ...
    def __init__(
        ...
        self.final_layer = RAEDDTFinalLayer(
        ...
        self.blocks = nn.ModuleList([
            RAEDDTBlock(self.encoder_hidden_size if i < self.num_encoder_blocks else self.decoder_hidden_size,
    ...
    def forward(self, hidden_states, timestep, class_labels, s=None, return_dict=True):
        ...
        if s is None:
            s = self.s_embedder(hidden_states)
            if self.use_pos_embed:
                s = s + self.pos_embed
        ...
        if not return_dict:
            return (x,)
        return RAEDDTTransformer2DModelOutput(sample=x)
