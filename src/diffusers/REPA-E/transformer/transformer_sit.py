...
class SiTTransformer2DModel(ModelMixin, ConfigMixin):
    """
    Scalable image Transformer (SiT) for REPA-E flow-matching diffusion.
    """
    ...
    def __init__(
        ...
        **block_kwargs,
        ...
        if z_dims is None:
            z_dims = [768]
        block_kwargs = {"qk_norm": qk_norm, "fused_attn": fused_attn, **block_kwargs}
    ...
    @torch.no_grad()
    def inference(self, x, t, y, force_drop_ids=None):
    ...
    def forward(
    ...
    ) -> Union[SiTTransformer2DModelOutput, torch.Tensor]:
        ...
        sample = self.inference(hidden_states, timestep, class_labels, force_drop_ids=force_drop_ids)
        if not return_dict:
            return sample
...
SIT_MODEL_PRESETS: Dict[str, Dict] = {
    "SiT-XL/1": dict(depth=28, hidden_size=1152, decoder_hidden_size=1152, patch_size=1, num_heads=16),
    "SiT-XL/2": dict(depth=28, hidden_size=1152, decoder_hidden_size=1152, patch_size=2, num_heads=16),
    "SiT-L/1": dict(depth=24, hidden_size=1024, decoder_hidden_size=1024, patch_size=1, num_heads=16),
    "SiT-L/2": dict(depth=24, hidden_size=1024, decoder_hidden_size=1024, patch_size=2, num_heads=16),
    "SiT-B/1": dict(depth=12, hidden_size=768, decoder_hidden_size=768, patch_size=1, num_heads=12),
    "SiT-B/2": dict(depth=12, hidden_size=768, decoder_hidden_size=768, patch_size=2, num_heads=12),
    "SiT-S/1": dict(depth=12, hidden_size=384, patch_size=1, num_heads=6),
    "SiT-S/2": dict(depth=12, hidden_size=384, patch_size=2, num_heads=6),
}


def build_sit_transformer(model_name: str, **kwargs) -> SiTTransformer2DModel:
    if model_name not in SIT_MODEL_PRESETS:
        raise ValueError(f"Unknown model {model_name}. Choose from {list(SIT_MODEL_PRESETS)}.")
    return SiTTransformer2DModel(**SIT_MODEL_PRESETS[model_name], **kwargs)
