...
@dataclass
class RepaeFlowMatchSchedulerOutput(BaseOutput):
    prev_sample: torch.FloatTensor


class RepaeFlowMatchScheduler(SchedulerMixin, ConfigMixin):
    """
    Flow-matching ODE/SDE scheduler used by REPA-E / SiT.

    Matches the original Euler and Euler-Maruyama samplers while exposing the
    standard Diffusers `set_timesteps` and `step` API.
    """
    ...
    def step(
        ...
        if not return_dict:
            return (prev_sample,)
        return RepaeFlowMatchSchedulerOutput(prev_sample=prev_sample)
