...
# decoder imported lazily to avoid circular imports


@dataclass
class RAEModelOutput(BaseOutput):
...
class RAEModel(ModelMixin, ConfigMixin):
    config_name = "config.json"
    ...
    def __init__(
        ...
        print(
            f"RAEModel: encoder={encoder_name}, resolution={resolution}, "
    ...
    ) -> Union[RAEModelOutput, Tuple[torch.Tensor, ...]]:
        z = self.encode(x)
        x_rec = self.decode(z)
        if not return_dict:
            if return_latent:
                return (x_rec, z)
            return (x_rec,)
        return RAEModelOutput(sample=x_rec, latent=z if return_latent else None)


# Backward-compatible alias used throughout training configs.
RAE = RAEModel
