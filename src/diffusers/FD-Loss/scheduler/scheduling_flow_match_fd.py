...
"""Flow-matching scheduler for FD-Loss JiT sampling (t: 1 -> 0)."""
...
class FDLossFlowMatchScheduler(SchedulerMixin, ConfigMixin):
    """Euler flow-matching scheduler with optional stochastic steps and generator-aware noise."""
    ...
    def set_timesteps(self, num_inference_steps: int, device: Union[str, torch.device, None] = None) -> torch.Tensor:
    ...
    def _resolve_step_index(self, timestep: Union[float, torch.Tensor, None]) -> int:
        ...
        if timestep is None:
            return 0
        t_value = float(timestep) if not isinstance(timestep, torch.Tensor) else float(timestep.reshape(-1)[0])
    ...
    ) -> Union[FDLossFlowMatchSchedulerOutput, Tuple[torch.Tensor]]:
        if self.timesteps is None:
            raise ValueError("Call `set_timesteps` before `step`.")
